...
export const GET = withAuth(async () => {
  try {
    ...
    const [
      ...
      // Orders (non-draft, non-cancelled) with items for seed demand analysis
      db
        .from("orders")
    ...
    const totalOrders = statusCountResults
      .filter((s) => s.status !== "DRAFT")
      .reduce((sum, s) => sum + s.count, 0);
    const confirmedOrders = statusCountResults.find((s) => s.status === "CONFIRMED")?.count ?? 0;
    ...
    for (const row of (dealerRows.data ?? []) as unknown as DealerRow[]) {
      ...
      if (existing) {
        existing.orderCount += 1;
        if (["CONFIRMED", "SHIPPED", "DELIVERED"].includes(row.status)) existing.confirmedCount += 1;
      } else {
        dealerMap.set(id, {
          name: row.dealer?.name ?? "Unknown",
          territory: row.dealer?.territory ?? null,
          orderCount: 1,
          confirmedCount: ["CONFIRMED", "SHIPPED", "DELIVERED"].includes(row.status) ? 1 : 0,
        });
      }
    }
