...
interface CreatableComboboxProps {
  ...
  value: string;
  /** Label to show in trigger when a "pending new" value is selected (no id yet). */
  pendingLabel?: string;
  onValueChange: (value: string) => void;
  /** Called when the user clicks "Create '...'" — receives the raw search string. */
...
export function CreatableCombobox({
  ...
  const displayLabel = selected?.label ?? pendingLabel;

  const filtered = search
    ? items.filter((i) => i.label.toLowerCase().includes(search.toLowerCase()))
    : items;

  // Show "Create" option when search text is non-empty AND doesn't exactly match any existing label
  const trimmed        = search.trim();
  const exactMatch     = items.some((i) => i.label.toLowerCase() === trimmed.toLowerCase());
  const showCreate     = trimmed.length > 0 && !exactMatch;

  function handleSelect(item: CreatableComboboxItem) {
      ...
      >
        ...
        <div className="max-h-52 overflow-y-auto py-1">
          {filtered.map((item) => (
            ...
            >
              <CheckIcon className={cn("size-3.5 shrink-0", item.value !== value && "invisible")} />
          ...
          {showCreate && (
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-1.5 text-sm text-left transition-colors hover:bg-accent hover:text-accent-foreground text-[var(--accent-foreground)]"
            >
              <PlusIcon className="size-3.5 shrink-0 text-foreground" />
              <span>
                Create{" "}
          ...
          {filtered.length === 0 && !showCreate && (
            <p className="px-3 py-4 text-center text-xs text-muted-foreground">No crops found</p>
          )}
        </div>
      </PopoverContent>
    </Popover>
